...
[Unit]
Description=Lunes Test Node Blockchain
...
install_or_update () {
    ...
    [ ! -f /etc/lunesnode/lunes_testnet.conf ] && UPDATE=0
}
...
echo " 	* Criar /etc/lunesnode e colocar o lunes_testnet.conf"
...
try $WGET --no-cache "${lunesnode_url}/LunesNode/releases/download/0.1.2/LunesNode-test-release-0.1.2.jar"  &> /dev/null
next
...
step "Configurando /etc/lunesnode/lunes_testnet.conf...."
mkdir /tmp/node
cd /tmp/node
try $WGET --no-cache "${lunesnode_git}/lunes_testnet.conf"  &> /dev/null
next
...
step "Incluindo IP lunes_testnet.conf...."
try sed -i "s/IPV4/$IPV4_FINAL/g" /tmp/node/lunes_testnet.conf
next

step "Incluindo senha da Wallet no lunes_testnet.conf....."
try sed -i "s/WALLET_PASS/$WALLET_PASS_FINAL/g" /tmp/node/lunes_testnet.conf
next

step "Incluindo seu SEED no lunes_testnet.conf ....."
get_data seed_hash
try sed -i "s/WALLET_SEED/$VALOR_FINAL/g" /tmp/node/lunes_testnet.conf
next
mv /tmp/node/lunes_testnet.conf /etc/lunesnode/lunes_testnet.conf
